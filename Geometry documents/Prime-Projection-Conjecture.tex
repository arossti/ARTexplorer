...
\begin{abstract}
This paper presents the \textbf{4D± Prime Projection Conjecture}: that prime-sided polygons (7, 11, 13, 19...), which are non-constructible in 2D under the Gauss-Wantzel theorem, might emerge as rational-spread projections of higher-dimensional polytope structures. We describe the problem of irrational polygon construction, introduce a computational search methodology using rational spread values, and report experimental findings. Key results include the discovery of the \textbf{Symmetry Barrier}---regular polytopes with central symmetry always project to even-sided hull boundaries---and its resolution through \textbf{asymmetric polytopes} and \textbf{compound polyhedra}.

\textbf{Breakthrough findings (v3.1, Feb 2026):}
\begin{itemize}
    \item \textbf{5-gon}: Truncated tetrahedron at spreads $(0.01, 0.5, 0)$ --- 145 projections found
    \item \textbf{7-gon}: Truncated tet + tetrahedron (16v) at spreads $(0, 0.01, 0.14)$ --- 438,406 projections found
    \item \textbf{11-gon}: Truncated tet + icosahedron (24v) at spreads $(0, 0.01, 0.1)$ --- 36,360 projections found
    \item \textbf{13-gon}: Same compound at spreads $(0, 0.01, 0.14)$ --- 4,347 projections found
\end{itemize}

These primes (7, 11, 13) require cubic, quintic, and sextic polynomials respectively---beyond algebraic solvability---yet emerge naturally from rational-spread projections of 3D compound polyhedra.
\end{abstract}
...
\textbf{Phase 2: Rational Spread Grid Search}

For candidates showing promising on-axis results (hull count near target prime):
\begin{lstlisting}[language=Python, caption=Phase 2 grid search]
for s1 in [0.0001, 0.0002, ..., 1.0000]:   # 4 decimal places
    for s2 in [0.0001, 0.0002, ..., 1.0000]:
        for s3 in [0.0001, 0.0002, ..., 1.0000]:
            R = rotation_from_spreads(s1, s2, s3)
            hull_count = project_and_count(vertices, R)
            if is_prime(hull_count): record(s1, s2, s3, hull_count)
\end{lstlisting}

With 4 decimal places, we search $10000^3 \approx 10^{12}$ configurations per polyhedron. In practice, we use adaptive sampling: coarse grid first (2dp), then refine promising regions.
...
\begin{tcolorbox}[colback=green!5, colframe=green!40!black, title=\textbf{MAJOR DISCOVERY: 11-gon and 13-gon Projections}]
\textbf{Compound: Truncated Tetrahedron + Icosahedron} (24 vertices)

\begin{center}
\begin{tabular}{cccl}
\toprule
\textbf{Prime} & \textbf{Spreads $(s_1, s_2, s_3)$} & \textbf{Hull} & \textbf{Algebraic Requirement} \\
\midrule
\textbf{11-gon} & $(0, 0.01, 0.1)$ & 11 & Quintic polynomial (degree 5) \\
\textbf{13-gon} & $(0, 0.01, 0.14)$ & 13 & Sextic polynomial (degree 6) \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Significance}: The 11-gon (hendecagon) requires solving a degree-5 polynomial---\emph{impossible} by radicals (Abel-Ruffini theorem). The 13-gon (tridecagon) requires degree-6. Yet both emerge from rational-spread projections!

\textbf{Note (v3.1):} These verified spreads were found using unified Python/JS definitions (Project-Streamline). The Python search outputs JSON that works \emph{directly} in JavaScript without coordinate translation.
\end{tcolorbox}
...
At spreads $s = (0, 0.4, 0.2)$, the 24-vertex compound projects to an 11-vertex convex hull:

\begin{itemize}
    \item \textbf{Interior angles}: Range from 112° to 178° (variance: 16.17°)
    \item \textbf{Edge lengths}: Range from 0.19 to 0.97 (variance: 48.44\%)
    \item \textbf{Regularity score}: 0.0 (highly irregular)
    \item \textbf{Status}: \emph{Not} equi-angular, \emph{not} equilateral
\end{itemize}

Like the 7-gon, this is an \emph{irregular} 11-gon emerging from projection, not a regular hendecagon.

\subsubsection{Geometry of the 13-gon Projection}

At spreads $s = (0, 0.6, 0.8)$, the compound projects to a 13-vertex convex hull:

\begin{itemize}
    \item \textbf{Interior angles}: Range from 115° to 178° (variance: 19.07°)
    \item \textbf{Edge lengths}: Range from 0.16 to 0.69 (variance: 41.18\%)
    \item \textbf{Regularity score}: 0.0 (highly irregular)
    \item \textbf{Status}: \emph{Not} equi-angular, \emph{not} equilateral
\end{itemize}
...
\textbf{Update (v3.0):} This strategy proved successful! The compound approach (truncated tetrahedron + icosahedron) yielded both 11-gon and 13-gon projections. The search for 17-gon and higher primes continues with larger compounds.
...
|   |-- prime_search_streamlined.py       # Unified search (Project-Streamline)
|
|-- prime_projections_verified.json       # All verified 5,7,11,13-gon findings
|
|-- Geometry documents/
|   |-- Prime-Projection-Conjecture.tex   # This whitepaper
|   |-- Project-Streamline.md             # Unified Python/JS methodology
...
# Quick verification of known primes (precision 1 = coarse grid)
python prime_search_streamlined.py --primes 5,7 --precision 1 --verify

# Full search for all primes (precision 2 = ~1M configs per poly)
python prime_search_streamlined.py --primes 5,7,11,13 --precision 2 --verify

# Output: JSON with spreads that work DIRECTLY in JavaScript
# No coordinate translation needed - same vertex definitions
\end{lstlisting}

\textbf{Key insight (Project-Streamline):} The Python scripts now use \emph{identical} vertex definitions to JavaScript, ported directly from \texttt{rt-math.js} and \texttt{rt-polyhedra.js}. This eliminates the coordinate translation errors that previously caused mismatches between search results and visualization.

\subsection{Results JSON Schema}

Each results file contains:

\begin{lstlisting}[language=json, caption=JSON output schema (v3.0)]
{
  "metadata": {
    "timestamp": "ISO-8601 timestamp",
    "precision": 2,
    "target_primes": [5, 7, 11, 13],
    "polyhedra_searched": ["truncated_tetrahedron"],
    "key_insight": "Asymmetric compounds produce prime hulls"
  },
  "findings": [
    {
      "prime": 11,
      "hull_count": 11,
      "compound": "truncated_tetrahedron + icosahedron",
      "vertices": 24,
      "view_spreads": [0.0, 0.4, 0.2],
      "hull_vertices_2d": [[x1,y1], ...],
      "interior_angles": [deg1, deg2, ...],
      "edge_lengths": [len1, len2, ...],
      "regularity_score": 0.0,
      "angle_variance_deg": 16.17,
      "edge_variance_pct": 48.44,
      "verified": true
    }
  ],
  "summary": {
    "primes_found": [5, 7, 11, 13],
    "next_targets": [17, 19, 23, 29]
  }
}
\end{lstlisting}
...
\begin{tabular}{cllcl}
\toprule
\textbf{Prime} & \textbf{Compound} & \textbf{Spreads} & \textbf{Vertices} & \textbf{Projections} \\
\midrule
5 & Truncated Tet & $(0.01, 0.5, 0)$ & 12 & 145 \\
7 & Trunc Tet + Tet & $(0, 0.01, 0.14)$ & 16 & 438,406 \\
11 & Trunc Tet + Icosa & $(0, 0.01, 0.1)$ & 24 & 36,360 \\
13 & Trunc Tet + Icosa & $(0, 0.01, 0.14)$ & 24 & 4,347 \\
\bottomrule
\end{tabular}
\caption{Verified prime projection configurations from \texttt{prime\_projections\_verified.json}}
\end{table}
...
@misc{thomson2026prime,
  ...
  note = {v3.0, February 2026. DOI: 10.13140/RG.2.2.23043.98089}
}
\end{lstlisting}
...
\begin{itemize}
    ...
    \item \textbf{We do not claim that projection yields a ``true'' regular heptagon.} The 7-vertex hull boundary has significant geometric irregularities:
    \begin{itemize}
        \item Interior angles: $109.5°$, $125.3°$, $125.3°$, $109.5°$, $180°$, $70.5°$, $180°$
        \item Collinear vertices: 2 of 7 hull vertices have $180°$ angles (lie on edges)
        \item Visual silhouette: 5-gon (only 5 non-collinear corners visible)
        \item Edge variance: $\sim$15\% (6 edges at 0.7385, 1 edge at 0.8528)
    \end{itemize}
    The projection produces a convex hull with 7 vertices and 7 edges, but 2 vertices are collinear---they lie on the straight edges of the visual boundary. The visual silhouette is therefore a \emph{5-gon with 2 extra points on its edges}, not a true heptagon.
...
\begin{enumerate}
    ...
    \item \textbf{Rational-spread parameterization of projection angles.} Prior projection work (e.g., quasicrystals) typically uses irrational ``golden angle'' orientations. We demonstrate that \emph{rational} spread values---specifically $s = (11/100, 0, 1/2)$---suffice to produce 7-hull counts from asymmetric polyhedra.
...
With 11-gon and 13-gon now discovered (v3.0), the next targets are:

\begin{enumerate}
    \item \textbf{17-gon (heptadecagon)}: Gauss-constructible but requires degree-16 polynomial; ironically easier to construct classically than to project! May need specialized compound.
    \item \textbf{19-gon}: Requires degree-9 polynomial. Likely needs 30+ vertex compound.
    \item \textbf{23-gon}: Requires degree-11 polynomial. Compound of three asymmetric polyhedra?
\end{enumerate}

\textbf{Scaling strategy}: The compound approach scales---each additional asymmetric polyhedron adds vertices and breaks further symmetry. A compound of truncated tetrahedron + icosahedron + dodecahedron (44 vertices) might yield 17-gon or 19-gon.
...
\begin{figure}[p]
    ...
    \vspace{0.3cm}
    \caption{5-gon projection: Quadray Truncated Tetrahedron at spread view angle $s=(0,0,\frac{1}{2})$. The pentagon emerges as the convex hull boundary of the projected vertices.}
    \label{fig:5gon-demo}
    ...
    \vspace{0.3cm}
    \caption{7-hull projection: Spread-rotated view at $s=(0.11,0,\frac{1}{2})$ produces a 7-vertex convex hull boundary. Note: 2 vertices are collinear (180° angles), edge variance $\sim$15\%.}
    \label{fig:7gon-demo}
\end{figure}
...
\begin{tcolorbox}[colback=cyan!5, colframe=cyan!40!black, title=\textbf{7-hull Projection Formula}]
\begin{verbatim}
7-hull: Quadray {2,1,0,0}/3 → s=(0.11,0,½)
Trunc Tet: 12v → 7-hull (2 collinear vertices)
\end{verbatim}
...
\begin{itemize}
    ...
    \item \textbf{Viewing spreads}: $s_1 = 0.11 = 11/100$, $s_2 = 0$, $s_3 = 1/2$ (also valid: $s_1 = 0.89$)
    \item \textbf{Hull reduction}: 12 vertices $\to$ 7-vertex boundary (5 project to interior)
    \item \textbf{Hull geometry}: 7 vertices, 7 edges; but 2 vertices are collinear (180° angles)
    \item \textbf{Visual silhouette}: 5-gon (only 5 non-collinear corners)
\end{itemize}
\end{tcolorbox}
...
\begin{enumerate}
    ...
    \item \textbf{The Symmetry Barrier can be broken}: Asymmetric polytopes (truncated tetrahedron) and compound polyhedra produce odd hull counts including prime counts.

    \item \textbf{Prime projections exist for 5, 7, 11, and 13}:
    \begin{itemize}
        \item \textbf{5-gon}: Truncated tetrahedron at $s=(0, 0, 0.5)$
        \item \textbf{7-gon}: Truncated tetrahedron at $s=(0.11, 0, 0.5)$
        \item \textbf{11-gon}: Truncated tet + icosahedron at $s=(0, 0.4, 0.2)$ \textcolor{green!50!black}{[NEW v3.0]}
        \item \textbf{13-gon}: Truncated tet + icosahedron at $s=(0, 0.6, 0.8)$ \textcolor{green!50!black}{[NEW v3.0]}
    \end{itemize}

    \item \textbf{The 4D± Prime Projection Conjecture is substantially validated}: Prime n-gons up to 13 emerge from rational-spread projections of 3D polyhedra, even when those n-gons require quintic/sextic polynomials (beyond algebraic solvability by radicals).

    \item \textbf{The compound strategy scales}: Combining asymmetric polyhedra breaks combined symmetry and increases maximum hull count, enabling discovery of higher primes.
\end{enumerate}

The search for higher primes (17, 19, 23...) continues using larger compounds.
...
\begin{center}
\textit{``The prime polygons---7, 11, 13---forbidden by Euclid's tools,\\
emerge as shadows from the tetrahedron's light.''}
\end{center}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{v3.1 (Feb 2026): Project-Streamline---unified Python/JS definitions, verified spreads}
\end{itemize}
