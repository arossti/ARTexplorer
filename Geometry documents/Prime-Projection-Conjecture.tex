...
\begin{itemize}
    ...
    \item \textbf{We do not claim that projection yields a ``true'' regular heptagon.} The 7-vertex hull boundary is not equilateral or equiangular in the projected plane---it is merely a 7-sided convex polygon.
...
\begin{enumerate}
    ...
    \item \textbf{Rational-spread parameterization of projection angles.} Prior projection work (e.g., quasicrystals) typically uses irrational ``golden angle'' orientations. We demonstrate that \emph{rational} spread values---specifically $s = (3/20, 0, 1/2)$---suffice to produce prime hull counts.
...
\begin{figure}[p]
    ...
    \vspace{0.3cm}
    \caption{7-gon projection: Spread-rotated view at $s=(0.11,0,\frac{1}{2})$ produces a heptagonal silhouette---a non-constructible prime polygon from rational parameters.}
    \label{fig:7gon-demo}
\end{figure}
