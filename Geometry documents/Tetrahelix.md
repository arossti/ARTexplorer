...
**"Unzipped" Mode (Future Implementation):**
Secondary strands would follow their own face-to-face chains parallel to the primary:
1. Bond first tetrahedron to an exposed face of the seed
2. Continue the secondary strand using its own face cycling pattern
3. Each strand maintains its own trajectory alongside the primary

This would be more DNA-like, where the two strands wind around each other.
