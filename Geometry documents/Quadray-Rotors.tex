...
\textbf{Quadray Rotors} escape differently: we use four tetrahedral directions instead of three orthogonal ones. Picture a tetrahedron at the origin, with axes pointing to its four vertices. These four directions naturally span a 4-dimensional space---not by adding an abstract fourth axis, but because four non-coplanar points require four independent coordinates to describe fully. There is no 2D surface to comb, so there is no cowlick to create.
...
\begin{itemize}
    ...
    \item \textbf{Phase 6.3}: Integrate into demo, test arbitrary-axis rotation via basis composition
