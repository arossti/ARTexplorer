...
\begin{enumerate}
    \item \textbf{Composition algebra:} The F,G,H matrix approach provides native Quadray rotation. Can we derive a closed-form ``product'' rule analogous to Hamilton's, or is matrix multiplication the natural operation?

    \item \textbf{Normalization trade-offs:} Operating without zero-sum normalization preserves 4 DOF and avoids gimbal lock. Are there numerical stability concerns that would require some form of constraint?

    \item \textbf{Optimal interpolation:} Is linear interpolation in $\mathbb{R}^4$ truly sufficient, or does the tetrahedral geometry suggest a more natural interpolation path?

    \item \textbf{Physical interpretation:} What does a ``rotation by spread $3/4$'' mean geometrically in tetrahedral terms? The spread corresponds to $60°$ or $120°$---both significant angles in tetrahedral symmetry.

    \item \textbf{Performance:} Can F,G,H rotations be made GPU-efficient, or is conversion to quaternions at the rendering boundary always necessary?
\end{enumerate}
