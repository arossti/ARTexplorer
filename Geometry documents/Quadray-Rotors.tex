...
\title{Spread-Quadray Rotors -- v1.1 (Feb 2026)\\
...
The Spread-Quadray Rotor system is implemented with the following components:
...
\midrule
\texttt{QuadrayRotor} class & Implemented & Partial\\
...
\texttt{multiply()} composition & Implemented & No (Hamilton product)\\
\texttt{toMatrix3()} output & Implemented & \textbf{Yes} (polynomial)\\
F,G,H native rotation & \textbf{Not yet} & Target\\
\bottomrule
\end{tabular}
\caption{Implementation status of Quadray rotor components}
\end{table}

\subsection{The Scaffolding Approach}

The current implementation uses Hamilton product (quaternion multiplication) internally while establishing the RT-pure API structure. This is a deliberate ``scaffolding'' approach:

\begin{enumerate}
    \item The \textbf{entry point} is RT-pure: spread $s$ instead of angle $\theta$
    \item The \textbf{structure} supports future RT-pure internals
    \item The \textbf{output} (matrix elements) is polynomial---no transcendentals
    \item The \textbf{composition algebra} currently uses Hamilton product as a crutch
\end{enumerate}

\begin{tcolorbox}[colback=blue!5, colframe=blue!40, title=\textbf{Honest Disclosure}]
``Use the crutches while building, then throw them away when ready!''

The Hamilton product produces \emph{correct} rotations. The goal is not to replace it with something that produces different results, but to express the same rotations using native tetrahedral algebra (F,G,H matrices) rather than orthogonal quaternion algebra.
\end{tcolorbox}

\subsection{Next Steps: Native Tetrahedral Rotation}

The path to fully RT-pure Quadray rotation:

\begin{enumerate}
    \item Implement F,G,H coefficient calculation from spread
    \item Replace \texttt{multiply()} with 4×4 matrix multiplication
    \item Verify output matches current Hamilton-based implementation
    \item Remove quaternion scaffolding entirely
\end{enumerate}

The mathematics already exists (Tom Ace's formulas, rationalized to RT-pure form). What remains is implementation and verification.
...
\textbf{Key demonstration:} In Euler mode, approaching the gimbal lock zones causes visible rotation instability (simulated jitter). In Quadray mode, the same axis orientations produce smooth, continuous rotation---the zones are ``irrelevant'' because the 4D representation has no singularities.
...
A verification test suite (\texttt{modules/fgh-verification-test.js}) was developed to compare F,G,H rotation in Quadray space against quaternion rotation in Cartesian space. The test protocol:
...
Y-axis (all angles) & \textcolor{green!50!black}{\textbf{PASS}} & $\sim 10^{-16}$ & Same pattern works\\
X-axis (all angles) & \textcolor{red}{\textbf{FAIL}} & $\sim 1.0$ & Naive extension fails\\
Z-axis (all angles) & \textcolor{red}{\textbf{FAIL}} & $\sim 1.0$ & Naive extension fails\\
\bottomrule
\end{tabular}
\caption{Phase 6.0 verification test results (February 2026)}
\end{table}

\subsection{Key Findings}

\begin{tcolorbox}[colback=green!5, colframe=green!40!black, title=\textbf{Phase 6.0 Complete: Tom Ace's F,G,H Formula Verified}]
\textbf{Outcome 1 confirmed for W and Y axes}: F,G,H rotation produces \emph{identical} results to quaternion rotation about the same tetrahedral axis, to machine precision ($10^{-16}$).

This means native Quadray rotation is \textbf{mathematically equivalent} to quaternion rotation when rotating about the tetrahedral basis directions---an elegant coordinate-native expression of the same underlying rotations.
\end{tcolorbox}
...
\begin{enumerate}
    ...
    \item \textbf{Y-axis also works}: The same circulant matrix pattern applies to Y-axis rotation.

    \item \textbf{X,Z-axis matrices need derivation}: The naive extension of placing F,G,H in a circulant pattern for X and Z axes produces incorrect rotations. Two solutions:
    \begin{itemize}
        \item Derive axis-specific matrix structures for X and Z
        \item Use conjugation: transform so target axis $\to$ W, apply $R_W$, transform back
    \end{itemize}

    \item \textbf{Same-axis composition verified}: $R_W(45°) \times R_W(45°) = R_W(90°)$ exactly, confirming that sequential F,G,H rotations compose correctly.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Phase 6.1}: Implement W-axis F,G,H rotation in \texttt{rt-math.js} (verified working)
    \item \textbf{Phase 6.2}: Derive correct X,Y,Z axis matrix structures (or implement conjugation)
    \item \textbf{Phase 6.3}: Verify arbitrary-axis rotation via basis decomposition
...
\begin{enumerate}
    ...
    \item \textbf{X,Z-axis matrix structure:} What is the correct F,G,H placement for X and Z axes? The circulant pattern that works for W and Y does not directly extend.

    \item \textbf{Arbitrary axis rotation:} Can we decompose arbitrary axis rotation into W,X,Y,Z compositions? Does this introduce ``tetrahedral gimbal lock''?

    \item \textbf{Normalization trade-offs:} Operating without zero-sum normalization preserves 4 DOF. Are there numerical stability concerns?
    ...
    \item \textbf{GPU efficiency:} Can F,G,H 4×4 matrices be sent directly to shaders, or is quaternion conversion at the boundary necessary?
\end{enumerate}
